...
async function getClobClient(config: AppConfig) {
  ...
  if (clobInitFailed) throw new Error("CLOB client init previously failed");
...
export async function executeLive(signal: Signal, config: AppConfig): Promise<Position> {
  ...
  const tokenSize = Math.floor(signal.size / signal.marketPrice);
  ...
  logger.info(
    {
      ...
      price: signal.marketPrice.toFixed(3),
